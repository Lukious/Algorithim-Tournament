...
Prob : https://tech.kakao.com/2018/09/21/kakao-blind-recruitment-for2019-round-1/
...
채팅방은 중복 닉네임을 허용하기 때문에, 현재 채팅방에는 Prodo라는 닉네임을 사용하는 사람이 두 명이 있다. 이제, 채팅방에 두 
번째로 들어왔던 Prodo가 Ryan으로 닉네임을 변경하면 채팅방 메시지는 다음과 같이 변경된다.
...
def solution(record):
    ...
    for command in record:
        clist = command.split(' ')
        if clist[0] == 'Enter':
            userDict[clist[1]] = clist[2]
            cLog.append([enterMsg,clist[1]])
        elif clist[0] == 'Leave':
            cLog.append([exitMsg,clist[1]])
        elif clist[0] == 'Change':
            userDict[clist[1]] = clist[2]
    for log in cLog:
